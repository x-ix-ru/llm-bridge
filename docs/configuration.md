...
- [OpenCode Config Restore / Восстановление конфига opencode](#opencode-config-restore--восстановление-конфига-opencode)
...
## OpenCode Config Restore / Восстановление конфига opencode

The `/opencode/config` POST endpoint accepts a user's current opencode.jsonc (possibly broken or outdated) and returns a fresh working configuration with the llm-bridge provider.

This is useful when opencode breaks and you need to quickly restore a working config.

### Endpoint / Эндпоинт

| Method | Endpoint | Description |
|---|---|---|
| `POST` | `/opencode/config` | Restore opencode config |

### Request / Запрос

- **Content-Type**: `application/jsonc` (or `application/json`)
- **Body**: JSONC or JSON — the user's current opencode config (can be empty `{}` or completely broken, it will be replaced)
- **Query Parameter**: `base_url` (optional) — override for the base URL

#### Example / Пример

```bash
curl -X POST 'http://localhost:8080/opencode/config?base_url=http://my-server:8080' \
  ...
  -d '{
    ...
    "provider": {
      "my-external-provider": {
        ...
        "models": {
          "gpt-4": { "id": "gpt-4", "limit": { "context": 8192, "input": 4000, "output": 4192 } }
        }
      }
    },
    "enabled_providers": ["my-external-provider"]
  }'
```
...
- **Body**: Fresh opencode.jsonc with only the `llm-bridge` provider enabled

The response replaces the user's provider with the llm-bridge provider containing all discovered models. The `enabled_providers` list contains only `["llm-bridge"]`.

#### Response Schema / Схема ответа

```jsonc
// Auto-generated by llm-bridge
// Only the "llm-bridge" provider is enabled — all external providers are implicitly disabled.
// Set "baseURL" below to match your llm-bridge address if different.

{
  "$schema": "https://opencode.ai/config.json",
  "enabled_providers": ["llm-bridge"],
  "provider": {
    "llm-bridge": {
      "api": "openai",
      "name": "llm-bridge",
...
- The user's config is parsed (JSONC comments stripped) — if parsing fails, returns `400 Bad Request`.
- All user's providers and models are **replaced** with the llm-bridge provider.
- `enabled_providers` is set to `["llm-bridge"]`.
- Model details (context limits) are derived from discovery metadata.
- Base URL resolution order:
  1. Query parameter `?base_url=...`
  2. Config `global.opencode_base_url`
  3. Auto-detected from request (`http://host` or `https://host`)
- The `/v1` suffix is appended to the base URL.

### Error Cases / Ошибки

| Status | Condition |
|---|---|
| `400 Bad Request` | Invalid JSON body (after stripping JSONC comments) |
| `500 Internal Server Error` | Failed to generate config (marshal error) |
